    ...
    if ser.in_waiting > 0:
        try:
            ...
            if linha_serial:
                ...
                if len(valores) == 2:
                    val1 = float(valores[0])
                    val2 = float(valores[1])
                    ...
                    y1_dados.append(val1)
                    y2_dados.append(val2)
                    
                    linha1.set_data(x_dados, y1_dados)
...
ani = FuncAnimation(fig, atualizar, interval=20, cache_frame_data=False)
plt.show()
ser.close()